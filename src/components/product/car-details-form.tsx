...
const fuelOptions = ['Petrol', 'Diesel', 'CNG & Hybrids', 'LPG', 'Electric'];
...
export function CarDetailsForm({ category, subcategory, onSubmit }: CarDetailsFormProps) {
      ...
      <Card>
        <CardContent className="p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="space-y-3">
              <label className="font-semibold text-sm" htmlFor="year">Year *</label>
              <Input id="year" type="number" value={details.year} onChange={handleChange} required />
            </div>

            <div className="space-y-3">
              <label className="font-semibold text-sm">Fuel *</label>
              <div className="flex flex-wrap gap-2">
                {fuelOptions.map(f => (
                  <Button key={f} type="button" variant={details.fuel === f ? 'default' : 'outline'} onClick={() => handleSelect('fuel', f)}>{f}</Button>
                ))}
              </div>
            </div>

            <div className="space-y-3">
              <label className="font-semibold text-sm">Transmission *</label>
              <div className="flex flex-wrap gap-2">
                {transmissionOptions.map(t => (
                  <Button key={t} type="button" variant={details.transmission === t ? 'default' : 'outline'} onClick={() => handleSelect('transmission', t)}>{t}</Button>
                ))}
              </div>
            </div>

            <div className="space-y-3">
              <label className="font-semibold text-sm" htmlFor="kmDriven">KM driven *</label>
              <Input id="kmDriven" type="number" value={details.kmDriven} onChange={handleChange} required />
            </div>
            ...
            <Button type="submit" size="lg" disabled={!details.brand || !details.year || !details.fuel || !details.transmission || !details.kmDriven || !details.adTitle || !details.description || !details.price}>Post now</Button>
          </form>
        </CardContent>
      </Card>
    </div>
